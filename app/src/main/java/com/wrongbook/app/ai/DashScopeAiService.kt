...
class DashScopeAiService(
    ...
    override suspend fun generateDetailedExplanation(question: Question): String {
        ...
        return cleanExplanationText(result.content)
    }
    ...
    override suspend fun followUp(question: Question, userMessage: String): FollowUpChat {
        ...
        return FollowUpChat(
            ...
            role = "assistant",
            content = cleanExplanationText(result.content),
            ...
            try {
                return client.chatCompletion(
                    messages = listOf(
                        ...
                        DashScopeClient.ChatMessage("user", imageContent(imageDataUrl, userPrompt))
                    ),
    ...
    private suspend fun chatFollowUpWithQuestionImageFallback(
        ...
        if (imageDataUrl != null) {
            try {
                ...
                messages.add(DashScopeClient.ChatMessage("user", imageContent(imageDataUrl,
    ...
    private fun imageContent(imageDataUrl: String, text: String): List<Map<String, Any>> =
        listOf(
            mapOf("type" to "image_url", "image_url" to mapOf("url" to imageDataUrl)),
        ...
        return try {
            ...
            QuestionAnalysis(
                ...
                difficultyScore = normalizeDifficultyScore(
                    json.get("difficultyScore")?.asInt ?: json.get("difficulty_score")?.asInt
                ),
    ...
    private fun extractJsonBlock(text: String): String {
        ...
        extractFencedJsonBlock(trimmed)?.let { return it }
    ...
    private fun extractFirstJsonObject(text: String): String? {
        ...
        for (index in startIndex until text.length) {
            ...
            when (char) {
                ...
                '"' -> inString = !inString
                '{' -> { if (!inString) depth += 1 }
                '}' -> { if (!inString) { depth -= 1; if (depth == 0) return text.substring(startIndex, index + 1) } }
            }
        }
        return null
    }

    private fun normalizeDifficulty(value: String?): String = when (value?.trim()) {
        "简单", "基础", "较易", "容易" -> "简单"
        "困难", "难", "较难", "高难" -> "困难"
        "中等", "一般" -> "中等"
        else -> "中等"
    ...
    private fun normalizeDifficultyScore(value: Int?): Int = when {
        value == null -> 3
        value < 1 -> 1
        value > 5 -> 5
        else -> value
    ...
    private fun cleanExplanationText(text: String): String {
        ...
        val filtered = lines.take(3).filterNot { fillerPattern.matches(it.trim()) } + lines.drop(3)
        val compact = filtered.fold(mutableListOf<String>()) { acc, line ->
            val t = line.trim()
            if (t.isEmpty()) { if (acc.isEmpty() || acc.last().isNotEmpty()) acc.add("") }
            else acc.add(t)
            acc
        }.toMutableList()
        while (compact.isNotEmpty() && compact.first().isEmpty()) compact.removeAt(0)
        while (compact.isNotEmpty() && compact.last().isEmpty()) compact.removeAt(compact.size - 1)
        var result = compact.joinToString("\n").replace("\n{3,}".toRegex(), "\n\n").trim()
        // Remove Qwen thinking mode output tags
        val thinkEndTag = "<" + "/think>"
        val thinkingEndTag = "</thinking>"
        if (result.contains(thinkEndTag)) {
            val idx = result.indexOf(thinkEndTag)
            if (idx >= 0) result = result.substring(idx + thinkEndTag.length).trim()
        } else if (result.contains(thinkingEndTag)) {
            val idx = result.indexOf(thinkingEndTag)
            if (idx >= 0) result = result.substring(idx + thinkingEndTag.length).trim()
        }
        return result
    }
}